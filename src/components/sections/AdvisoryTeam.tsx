...
const AdvisoryTeam: React.FC = () => {
  ...
  return (
    <div>
      <h2 className="mb-4 text-center">Advisory Team</h2>
      <div className="row justify-content-center">
        {advisoryTeam.map((advisor, idx) => (
          <div className="col-md-4 col-12 d-flex flex-column align-items-center mb-4" key={advisor.name}>
            ...
            <h4 className="fw-bold text-center">{advisor.name}</h4>
            <div className="text-center text-muted mb-2" style={{ minHeight: 40 }}>{advisor.title}</div>
          </div>
        ))}
      </div>
      {/* Modal */}
